...
function Grid2D:init(iMax, jMax, debugName)
    rawset(
            self, "_dbg",
            (debugName and debugName .. ":" or "") .. tostring(iMax) .. "," .. tostring(jMax))
    assert(
            type(iMax) == "number" and iMax >= 1 and math.floor(iMax) == iMax,
            "Invalid iMax " .. self._dbg)
    assert(
            type(jMax) == "number" and jMax >= 1 and math.floor(jMax) == jMax,
            "Invalid jMax " .. self._dbg)
...
function Grid2D:get(i, j)
    if Grid2D._ijValid(self, i, j) then
        ...
        simlog(
                "[QEDCTRL] Out of bounds access to grid: %s,%s into %s\n%s", tostring(i),
                tostring(j), self._dbg, debug.traceback())
...
function Grid2D:set(i, j, v)
    if Grid2D._ijValid(self, i, j) then
        ...
        simlog(
                "[QEDCTRL] Out of bounds assignment to grid: %s,%s=%s into %s\n%s", tostring(i),
                tostring(j), tostring(v), self._dbg, debug.traceback())
        inputmgr.onControllerError()
    end
end
...
function Grid2D:_ijValid(i, j)
    ...
    return (type(i) == "number" and i >= 1 and i <= iMax and math.floor(i) == i and type(j) ==
                   "number" and j >= 1 and j <= jMax and math.floor(j) == j)
end
