...
function end_turn_dialog:init(game)
    ...
    screen.binder.pnl.binder.resumeBtn.onClick = util.makeDelegate(
            nil, onClickResult, self, RESULT_RESUME)
...
function end_turn_dialog:show()
    ...
    if self._game then
        ...
        local rewindsAvailable = (sim:getTags().rewindsLeft or 0) > 0 and
                                         not sim:getTags().isTutorial and
                                         self._game.hud:canShowElement("rewindBtn")
