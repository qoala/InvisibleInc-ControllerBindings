    ...
    if ctrlDef and ctrlDef.focusImages then
        ...
        local focusSize = (ctrlDef.focusImageSize or
                                  (ctrlDef.focusImagePadding and self._checkSize +
                                          ctrlDef.focusImagePadding) or (self._arrowSize * 1.2))
...
do
    ...
    function overrides:onActivate(screen)
        ...
        if ctrl:canCombobox() then
            self._qedctrl_ctrl = ctrl
        end
...
function mui_combobox:_updateImageState()
    ...
    if focusImage then
        local inFocus = not mouseMode and
                                (btnState == mui_button.BUTTON_Hover or btnState ==
                                        mui_button.BUTTON_Active)
...
function mui_combobox:_destroyDropDown()
    if self._listbox then
        ...
        if self._qedctrl_ctrl then
            self._qedctrl_ctrl:finishCombobox()
        end
