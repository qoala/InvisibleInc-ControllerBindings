...
function grid_layout:init(def, ...)
    ...
    assert(
            self._def.w and type(self._def.w) == "number" and self._def.h and type(self._def.h) ==
                    "number",
            "[QEDCTRL] Missing w,h (" .. tostring(self._def.w) .. "," .. tostring(self._def.h) ..
                    ") for grid layout " .. self._debugName)
    assert(
            type(self._def.children) == "table",
            "[QEDCTRL] Missing children for grid layout " .. self._debugName)
    ...
    for i, childDef in ipairs(self._def.children) do
        ...
        assert(
                type(t) == "table" and #t == 2,
                "[QEDCTRL] Invalid coord " .. tostring(t) .. " for grid child " ..
                        tostring(self._debugName) .. "/" .. (childDef.id or i))
        local x, y = t[1] * scale, t[2] * scale
        assert(
                not self:getChild(x, y),
                "[QEDCTRL] Duplicate coord " .. x .. "," .. y .. " for grid child " ..
                        tostring(self._debugName) .. "/" .. (childDef.id or i))

        local child = ctrl_layouts.createLayoutNode(
                childDef, self._navigatePath, self._debugName, i, x .. "," .. y)
        if child then
...
function grid_layout:_findChild(childID)
    ...
    if foundBinding then
        return foundBinding[1], foundBinding[2], foundBinding[3]
    end
...
local function shouldRecall(options, ctrlDef)
    ...
    local recallX = (ctrlDef.recallOrthogonalX and options.dir ~= ctrl_defs.LEFT and options.dir ~=
                            ctrl_defs.RIGHT)
    local recallY = (ctrlDef.recallOrthogonalY and options.dir ~= ctrl_defs.UP and options.dir ~=
                            ctrl_defs.DOWN)
...
function grid_layout:onFocus(options, childID, ...)
        ...
        if recallX or recallY then
            ...
            elseif recallX then
                ...
                if dir == ctrl_defs.UP then
                    y, ySign = self._h, DESC
                elseif dir == ctrl_defs.DOWN then
                    y, ySign = 1, ASC
                else
                    y, ySign = self._defaultY, self._defaultYNext
                end
            ...
            elseif recallY then
                local dir = options.dir
                local xSign
                if dir == ctrl_defs.LEFT then
                    x, xSign = self._w, DESC
                elseif dir == ctrl_defs.RIGHT then
                    x, xSign = 1, ASC
                else
                    x, xSign = self._defaultX, self._defaultXNext
                end
    ...
    local dir = options.dir
    if dir == ctrl_defs.UP then
        return self:_doFocus(
                options, self:_getOrNextXY(self._defaultX, self._defaultXNext, self._h, DESC))
    elseif dir == ctrl_defs.DOWN then
        return self:_doFocus(options, self:_getOrNextXY(self._defaultX, self._defaultXNext, 1, ASC))
    elseif dir == ctrl_defs.LEFT then
        return self:_doFocus(
                options, self:_getOrNextXY(self._w, DESC, self._defaultY, self._defaultYNext))
    elseif dir == ctrl_defs.RIGHT then
        return self:_doFocus(options, self:_getOrNextXY(1, ASC, self._defaultY, self._defaultYNext))
    end
...
function grid_layout:_getOrNextJI(j0, jSign, i0, iSign, bounceBack)
    local iMax, jMax = self:_xy2ij(self._w, self._h)
    if (not jSign or not i0 or i0 < 1 or i0 > iMax or not j0 or j0 < 1 or j0 > jMax) then
        simlog(
                "[QEDCTRL] Failed grid:navJI with invalid args %s%s,%s%s%s / %s,%s %s\n%s", i0,
                SIGN_DBG[iSign] or "=", j0, SIGN_DBG[jSign] or "?", bounceBack and "*" or "", iMax,
                jMax, self._debugName, debug.traceback())
...
function grid_layout:_getOrNextI(i0, iSign, j0)
    local iMax, jMax = self:_xy2ij(self._w, self._h)
    if (not i0 or i0 < 1 or i0 > iMax or not j0 or j0 < 1 or j0 > jMax) then
        simlog(
                "[QEDCTRL] Failed grid:navI with invalid args %s%s,%s= / %s,%s %s\n%s", i0,
                SIGN_DBG[iSign] or "=", j0, iMax, jMax, self._debugName, debug.traceback())
        inputmgr.onControllerError()
        return
    end
...
return {grid_layout = grid_layout, rgrid_layout = rgrid_layout, cgrid_layout = cgrid_layout}
