...
function list_layout:init(def, ...)
    ...
    assert(
            ...
            "[QEDCTRL] Missing children for list layout " .. self._debugName)
    local ctrl_layouts = include(SCRIPT_PATHS.qedctrl .. "/mui/ctrl_layouts")
    for i, childDef in ipairs(self._def.children) do
        assert(
                type(childDef.coord) == "number",
                "[QEDCTRL] Invalid coord " .. tostring(childDef.coord) .. " for child " ..
                        tostring(self._debugName) .. "/" .. (childDef.id or i))

        local child = ctrl_layouts.createLayoutNode(
                childDef, self._navigatePath, self._debugName, i, childDef.coord)
        if child then
...
function list_layout:_findChild(childID)
    ...
    if foundBinding then
        return foundBinding[1], foundBinding[2]
    end
...
return {list_layout = list_layout, vlist_layout = vlist_layout, hlist_layout = hlist_layout}
