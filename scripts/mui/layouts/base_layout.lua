...
function base_layout:init(def, parentPath, debugParent, debugCoord)
    ...
    simlog(
            "LOG_QEDCTRL", "ctrl:init %s:%s%s", tostring(self._debugName), tostring(self._SHAPE),
            debugCoord and " @" .. debugCoord or "")
...
function base_layout:onNav(navDir, coord, ...)
    if not coord and self._focusChild and self._focusChild:onNav(navDir) then
        return true
    end

    if self._onInternalNav and self:_onInternalNav(navDir, coord, ...) then
        return true
    end
...
function base_layout:onCommand(command, dat)
    if self._focusChild and self._focusChild:onCommand(command, dat) then
        return true
    end
    if dat.interrupted then
        return
    end
